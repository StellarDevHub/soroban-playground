...
fn test_initialize_twice_fails() {
    let (_env, owner, client) = setup();
    client.initialize(&owner).unwrap();
    assert_eq!(client.initialize(&owner).unwrap_err(), Error::AlreadyInitialized);
}
...
fn test_pauser_role_can_pause() {
    ...
    client.grant_role(&owner, &pauser, &(Role::Pauser as u32)).unwrap();
    client.pause(&pauser).unwrap();
    assert!(client.is_paused());
}
...
fn test_non_pauser_cannot_pause() {
    ...
    assert_eq!(client.pause(&rando).unwrap_err(), Error::Unauthorized);
}
...
fn test_grant_role_ok() {
    ...
    client.grant_role(&owner, &minter, &(Role::Minter as u32)).unwrap();
    assert!(client.has_role(&minter, &(Role::Minter as u32)));
}
...
fn test_grant_role_twice_fails() {
    ...
    client.grant_role(&owner, &minter, &(Role::Minter as u32)).unwrap();
    assert_eq!(
        client.grant_role(&owner, &minter, &(Role::Minter as u32)).unwrap_err(),
...
fn test_revoke_role_ok() {
    ...
    let minter = Address::generate(&env);
    client.grant_role(&owner, &minter, &(Role::Minter as u32)).unwrap();
    client.revoke_role(&owner, &minter, &(Role::Minter as u32)).unwrap();
    assert!(!client.has_role(&minter, &(Role::Minter as u32)));
}
...
fn test_revoke_ungranted_role_fails() {
    ...
    let rando = Address::generate(&env);
    assert_eq!(
        client.revoke_role(&owner, &rando, &(Role::Minter as u32)).unwrap_err(),
        Error::RoleNotHeld
    );
}
...
fn test_non_admin_cannot_grant_role() {
    ...
    assert_eq!(
        client.grant_role(&rando, &victim, &(Role::Minter as u32)).unwrap_err(),
        Error::Unauthorized
    );
}
...
fn test_admin_role_holder_can_grant() {
    ...
    client.grant_role(&owner, &admin2, &(Role::Admin as u32)).unwrap();
    // admin2 should now be able to grant minter
    let minter = Address::generate(&env);
    client.grant_role(&admin2, &minter, &(Role::Minter as u32)).unwrap();
    assert!(client.has_role(&minter, &(Role::Minter as u32)));
}
...
fn test_non_owner_cannot_transfer_ownership() {
    ...
    let new_owner = Address::generate(&env);
    assert_eq!(
        client.transfer_ownership(&rando, &new_owner).unwrap_err(),
        Error::Unauthorized
    );
}
...
fn test_grant_before_init_fails() {
    ...
    assert_eq!(
        client.grant_role(&caller, &grantee, &0).unwrap_err(),
...
fn test_custom_role_grant_revoke() {
    ...
    client.grant_role(&owner, &user, &custom_role).unwrap();
    assert!(client.has_role(&user, &custom_role));
    client.revoke_role(&owner, &user, &custom_role).unwrap();
    assert!(!client.has_role(&user, &custom_role));
}
