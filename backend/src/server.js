...
const globalLimiter = rateLimit({
  ...
  legacyHeaders: false,
  handler: (req, res) => {
    res.status(429).json({ status: 429, error: "Too Many Requests", message: "Too many requests from this IP, please try again later." });
  },
});
...
const compileLimiter = rateLimit({
  ...
  legacyHeaders: false,
  handler: (req, res) => {
    res.status(429).json({ status: 429, error: "Too Many Requests", message: "Compile endpoint rate limit exceeded. Please wait and try again later." });
  },
});
